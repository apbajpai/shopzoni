...
<div class="centercontent tables">
	<div class="pageheader notab">
		...
		<span><b><font size="3">Note : After Click on "Sell Yours" please activate the status and update price from Your Products (Tab) to show on shop.</font></b></span>
	</div><!--pageheader-->	
			...
			<tbody>
				...
				?>
					<tr class='gradeA'>
						...
						<?php if($data->seller_product_map->id==""){ ?>
						<td>
							<input type="button" name="sell_yours" id="sell_yours" value="sell yours" onclick="sell('<?php echo $data->id; ?>');">
							<div id="sellmsgDiv<?php echo $data->id; ?>" style="color:red; font-size:12px;"></div>
							<div id="sellsucessmsgDiv<?php echo $data->id; ?>" style="color:green; font-size:12px;"></div>
						...
						<?php }else{ ?>
						<td><div style="color:red; font-size:12px;">Already Added..!</div></td>
						<?php } ?>
				...
				<?php if($search['brand_id']!=""){ ?>
				<!--<tr class='gradeA'>
					<td rowspan="2" style="width:15%; vertical-align:middle; border-top: 1px solid #ddd;">
						<input type="submit" name="btnProduct" id="btnProduct" class="search_submit" value="Add Product" style="padding:15px 10px;">
						<input type="hidden" name="brand_id" id="brand_id" value="<?php echo $search['brand_id']; ?>">
					</td>
				</tr>-->
				<?php } ?>
			</form>
	...
	function sell(id){
		...
		.done (function(data) { 
			...
			if(str[1]=="false"){
				...
				jQuery('#sellsucessmsgDiv'+str[0]).html("");
				jQuery('#sellmsgDiv'+str[0]).html("Product Allready added"); 
			}else{
				jQuery('#sellsucessmsgDiv'+str[0]).html('<img src="<?php echo base_url(); ?>img/right_sign.png" height="48" width="48" />');
			}
			//alert(data);
