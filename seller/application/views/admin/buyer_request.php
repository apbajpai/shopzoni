			...
			<tbody>
				<?php $i=0;foreach($records as $data){$i++;?>
					<tr class='gradeA' id="requestDiv<?php echo $data->id; ?>">
						...
						<td>
						<input type="button" onclick="approveRequest('<?php echo $data->id; ?>')" name="request_status" id="request_status" value="Approve">
						<input type="button" onclick="rejectRequest('<?php echo $data->id; ?>')" name="request_status" id="request_status" value="Reject">
						</td>
