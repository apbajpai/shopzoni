...
class Cancel_order extends MY_Controller {
	...
	public function page($page_no)
	{
		...
		$per_page=50;
		$total_record	= $this->cancel_order_model->GetTotalRecord();		
		$config['base_url'] = site_url().'admin/cancel_order/index';
		...
		$array_records = $this->cancel_order_model->GetRecords($offset,$per_page);
	...
	public function approveOrderStatus(){
		$this->cancel_order_model->approveOrderStatus();
	...
	public function sendMailToBuyer(){
		...
		$this->cancel_order_model->sendMailToBuyer();		
		$array_records = $this->cancel_order_model->GetRecordsByOrderId($order_id);
	...
	public function rejectOrderStatus(){
		$this->cancel_order_model->rejectOrderStatus();
		echo $cart_id  =  trim($this->input->post('cart_id'));		
		//echo "<input type="button" onclick="changeOrderStatus(' echo $data->cart_id; ')" name="order_status" id="order_status" value="Approve">";
	}
	...
	public function view($order_id){
		...
		$this->cancel_order_model->changeViewStatus($order_id);
		$data['heading']='Cancel Order'; 
		$this->load->view('admin/header', $data);		
		$this->load->view('admin/sidebar');
		$array_records = $this->cancel_order_model->GetRecordsByOrderId($order_id);
		$data['records'] = $array_records; 
		$this->load->view('admin/cancel_view_order', $data);
		$this->load->view('admin/footer');
	}
}
?>