...
class Product_model extends CI_Model {
	...
	public function GetTotalRecord() {
			...
			$this->db->select('a.*,b.name as category_name,c.name as parent_name,d.name as sectin_name,f.name as brand_name,e.price,e.id as seller_map_id,e.status as seller_map_status');
	...
	public function GetRecordById($id) { 		
		$this->db->select('a.id as product_id,a.name,a.model_no,a.weight,a.unit,b.*,c.name as brand_name,a.mrp');
		$this->db->from("tbl_product as a");
	...
	public function getPacketRecord($seller_id,$product_id) {
		//$this->db->where('status', 1);		
		$this->db->where('seller_id', $seller_id);	
		$this->db->where('product_id', $product_id);	
	...
	public function GetRecordByModelNo() {		
		$model_no 	=  trim($this->input->post('model_no'));			
				
		$this->db->select('a.*,b.name as category_name,c.name as parent_name,d.name as sectin_name,f.name as brand_name');
		$this->db->from("tbl_product as a");
		$this->db->join('tbl_category as b', 'a.category_id = b.id', 'left');
		$this->db->join('tbl_category as c', 'b.parent_id = c.id', 'left');		
		$this->db->join('tbl_section as d', 'd.id = c.section_id', 'left');		
		...
		if($model_no!="")
		$this->db->where('a.model_no',$model_no);		
