...
class Category_model extends CI_Model {
	...
	public function GetParentRecords($start=0, $limit=0) {
		$name  =  trim($this->input->post('name'));
		$this->db->select('a.*,b.name as sectin_name');
		$this->db->from("tbl_category as a");				
		$this->db->join('tbl_section as b', 'b.id = a.section_id', 'left');		
		$this->db->where('a.parent_id', 0);		
		$this->db->where('a.status !=', 5);		
		if($name != '')
		$this->db->like('a.name',$name);
	...
	public function GetChildRecordsControl1($parent_id) {
        $this->db->where('parent_id', $parent_id);		
		$this->db->where('status', 1);
		$query = $this->db->get("tbl_category");
		//echo $this->db->last_query();
	...
	public function GetChildRecordsControlNew1() {
        $this->db->where('parent_id !=', 0);		
		$this->db->where('status', 1);
		$query = $this->db->get("tbl_category");
		//echo $this->db->last_query();
	...
	public function GetSellerCategory() {
		...
		$this->db->select('distinct(c.name),c.*,f.seller_id');
