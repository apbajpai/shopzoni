...
if (!function_exists('getCatSubcatTitle'))
{
	function getCatSubcatTitle($id){
		...
		$query = $CI->db->query("SELECT id, parent_id, name FROM tbl_product_category where id = '$id' LIMIT 1");
		$row = $query->row();
		if(($row->parent_id) != 0){
			$query1 = $CI->db->query("SELECT name FROM tbl_product_category where id = '$row->parent_id' LIMIT 1");
			$row1 = $query1->row();
